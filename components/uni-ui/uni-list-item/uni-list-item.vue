<template>
	...
	<cell>
		...
		<view
			:class="[{ 'uni-list-item--disabled': disabled }, { 'border-bottom': showBorderBottomLine }]"
		...
		>
			...
			<view class="uni-list-item__container" :class="{ 'container--right': showArrow || link, 'flex--direction': direction === 'column' }">
				...
				<slot name="footer">
					<view v-if="showRightSlot || rightText || showBadge || showSwitch" class="uni-list-item__extra" :class="{ 'flex--justify': direction === 'column' }">
 ...
 * @property {Boolean}  showBorderBottomLine = [true|false] 自定义 是否显示下边线 默认为 false
 * @property {String}  leftIcon 自定义左边 图标 icon
 * @property {String}  leftIconStyle 自定义左边 图标 样式
...
export default {
	...
	props: {
		...
		// 自定义 是否显示询问边框 默认: false
		showBorderBottomLine: {
			type: Boolean,
			default: false
		},
		// 显示右边插槽
